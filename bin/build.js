...
builder.prototype.init = function() {
	...
	//scope.file.removeDir(scope.base+"/"+scope.buildProfile.output, function() {
			...
			scope.loadTemplates(function() {
				...
				scope.file.listDirectories(scope.base+"/"+scope.buildProfile.src+"/"+scope.buildProfile.pages, function(directories) {
					...
					buildStack.process(function() {
						...
						// Copy the libs
						ncp(scope.base+"/"+scope.buildProfile.src+"/bower_components", scope.base+"/"+scope.buildProfile.output+"/libs", function() {
							console.log("Libs copied from ", scope.base+"/"+scope.buildProfile.src+"/bower_components", " to ", scope.base+"/"+scope.buildProfile.output);
						});
						
						// Copy the public dir
						ncp(scope.base+"/"+scope.buildProfile.src+"/public", scope.base+"/"+scope.buildProfile.output+"/public", function() {
							console.log("Public dir copied from ", scope.base+"/"+scope.buildProfile.src+"/public", " to ", scope.base+"/"+scope.buildProfile.output);
						});
					});
				});
			});
	...
	switch (type) {
		case ".js":
			_.each(list, function(file) {
				file = file.replace(scope.base+"/"+scope.buildProfile.src+"/bower_components", "libs");
